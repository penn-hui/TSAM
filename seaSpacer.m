...
disp(i);
    ...
    for j=1:n1
        ...
        if ind{1,1}(1,j)-21-fn>=1 && ind{1,1}(1,j)+1+bn<=length(seq{1,1})
            ...
            %spacer_info{1,7}='sense';
            spacer_info{1,13}=1;
            %spacer_info{1,14}=cut_g+ginfo{i,2}{1,3}-1;
            %spacer_info{1,15}=ginfo{i,2}{1,2};
    ...
    for j=1:n2
        ...
        if ind_a{1,1}(1,j)-21-fn>=1 && ind_a{1,1}(1,j)+1+bn<=length(antigSeq{1,1})
            ...
            spacer_info{1,13}=0;
            %spacer_info{1,14}=cut_g+ginfo{i,3}{1,3}-1;
            %spacer_info{1,15}=ginfo{i,2}{1,3};
            spacer_antisense=[spacer_antisense;spacer_info];
        end
        
    end
    %spacer_antisense=sortrows(spacer_antisense,3);
    spacer=[spacer_sense;spacer_antisense];
    spacer=sortrows(spacer,7);
    [m,n]=size(spacer);
    % spacer density
    for j=1:m
        if j==1
            spacer{j,n+1}=(spacer{j+1,8}-spacer{j,8})/2;
        elseif j==m
            spacer{j,n+1}=(spacer{j,8}-spacer{j-1,8})/2;
        else
            spacer{j,n+1}=(spacer{j+1,8}-spacer{j-1,8})/2;
        end
    end
    spacers=[spacers;spacer];
    end
end
